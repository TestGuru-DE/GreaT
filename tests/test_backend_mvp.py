...
def test_end_to_end_pairwise_generation(tmp_path):
    ...
    # 2) Kategorien & Werte
    r = client.post(f"/api/projects/{pid}/categories", json={"name": "Gewicht", "order_index": 0})
    assert r.status_code == 200
    cid_w = r.json()["id"]
    r = client.post(f"/api/categories/{cid_w}/values", json={"value": "500g"})
    assert r.status_code == 200
    r = client.post(f"/api/categories/{cid_w}/values", json={"value": "1000g"})
    assert r.status_code == 200

    r = client.post(f"/api/projects/{pid}/categories", json={"name": "Farbe", "order_index": 1})
    assert r.status_code == 200
    cid_f = r.json()["id"]
    r = client.post(f"/api/categories/{cid_f}/values", json={"value": "Rot"})
    assert r.status_code == 200
    r = client.post(f"/api/categories/{cid_f}/values", json={"value": "Blau"})
    assert r.status_code == 200
    ...
    r = client.post(f"/api/projects/{pid}/generate", json={"strategy": "pairwise"})
    assert r.status_code == 200, r.text
    ...
    # 6) CSV export
    r = client.get(f"/api/generations/{gen_id}/export/csv")
    assert r.status_code == 200
    assert r.headers["content-type"].startswith("text/csv")
    # CSV enthält tatsächliche Kategorienamen als erste Datenspalten
    assert "Gewicht" in r.text
    assert "Farbe" in r.text
